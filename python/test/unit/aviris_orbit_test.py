from emit import AvirisOrbit, AvirisTimeTable, AvirisIgm
import geocal
import scipy.optimize


def test_aviris_orbit(isolated_dir, aviris_test_data):
...
def test_aviris_igc(isolated_dir, aviris_test_data):
    ...
    cam = geocal.SimpleCamera(0, 0, 0, focal_length, line_pitch, sample_pitch, 1, nsamp)
    dem = geocal.SrtmDem()
    ...
    def f(x):
        igc.ipi.camera = geocal.SimpleCamera(
            0, 0, 0, x[0], line_pitch, -sample_pitch, 1, nsamp
        )
        res = []
        for ln in range(0, igm.shape[0], 100):
            for smp in range(0, igm.shape[1], 100):
                res.append(
                    geocal.distance(
                        igc.ground_coordinate(geocal.ImageCoordinate(ln, smp)),
                        igm[ln, smp],
                    )
                )
        return res
