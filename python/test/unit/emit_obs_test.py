...
def test_generate_obs(igc, isolated_dir, emit_loc):
    ...
    for smp in range(0, igc.number_sample, 100):
        ...
        assert g.view_zenith[ln, smp] == pytest.approx(p_zen, abs=0.1)
        assert g.view_azimuth[ln, smp] == pytest.approx(p_azm, abs=5.0)
        assert g.solar_zenith[ln, smp] == pytest.approx(p_szen, abs=0.01)
        assert g.solar_azimuth[ln, smp] == pytest.approx(p_saz, abs=0.01)
