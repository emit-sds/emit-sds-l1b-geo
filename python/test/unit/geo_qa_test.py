...
def test_geo_qa(isolated_dir, l1_osp_dir, test_data):
    ...
    # we run.
    run_dir = Path("/home/smyth/Local/emit-build/build/l1b_geo_run")
    ...
    igccol = geocal.read_shelve(str(run_dir / "igccol_initial.xml"))
    igccol_corrected = geocal.read_shelve(str(run_dir / "igccol_sba.xml"))
    tpcol = geocal.read_shelve(str(run_dir / "tpcol.xml"))
    for i in range(3):
        tplist = geocal.TiePointCollection(
            [tp for tp in tpcol if (tp.image_coordinate(i))]
        )
        ...
        geo_qa.add_tp_log(f"Image Index {i + 1} 0", run_dir / "tpmatch_00{i+1}.log_0")
        geo_qa.add_tp_single_scene(
            i,
            igccol,
            tplist,
            ntpoint_initial,
