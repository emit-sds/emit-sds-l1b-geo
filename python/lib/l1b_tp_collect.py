        ...
        try:
            ...
            for i2, tpcol in enumerate(self.tpcollect):
                ...
                if(len(res) >= self.min_tp_per_scene):
                    ...
                    logger.info("Removed %d tie-points using RANSAC for %s" % (len1-len(res), self.igccol.title(i)))
                if(len(res) >= self.min_tp_per_scene):
                    break
            number_match_try = i2 + 1
            if(len(res) < self.min_tp_per_scene):
                logger.info("Too few tie-point found. Found %d, and require at least %d. Rejecting tie-points for %s" % (len(res), self.min_tp_per_scene, self.igccol.title(i)))
                res = []
            else:
                logger.info("Found %d tie-points for %s try %d" % (len(res), self.igccol.title(i), number_match_try))
            logger.info("Done collecting tp for %s" % self.igccol.title(i))
        except Exception as e:
            logger.info("Exception occurred while collecting tie-points for %s",
                        self.igccol.title(i))
