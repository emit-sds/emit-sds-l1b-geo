...
class L1OspDir:
    ...
    def setup_spice(self):
        ...
        if v is None:
            raise RuntimeError(
                "Need to set spice_data_dir in the l1b_geo_config.py file"
            )
    ...
    CONFIG_VARIABLES = [
        ...
        [
            "do_final_projection",
            False,
            "Do a final projection of the scenes, useful for doing diagnostics",
        ],
        [
            "skip_sba",
            False,
            "Skip the SBA step, and just use the reported camera, ephemeris, attitude values without correction",
        ],
        [
            "use_scene_index",
            False,
            "Use the scene number for indexing scene based files. Otherwise we use the start time in the file name as the index",
        ],
        [
            "rad_match_scale",
            1000,
            "Scaling to use on radiance data to produce image used for image matching",
        ],
        ...
        [
            "map_number_subpixel",
            3,
            "Number of subpixels to use when resampling to a map",
        ],
        ["glt_rotated", True, "Generate GLT as a rotated map rather than North is up"],
        [
            "num_tiepoint_x",
            None,
            "Number of tiepoints to try for in x direction for each scene",
        ],
        [
            "num_tiepoint_y",
            None,
            "Number of tiepoints to try for in y direction for each scene",
        ],
        [
            "min_tp_per_scene",
            None,
            "Minimum number of tiepoints for a scene to consider the matching successful",
        ],
        ...
        [
            "ffthalf",
            2,
            "0 = no halving of fftsize near edge,1 = single halving of fftsize near edge,double halving of fftsize near edge",
        ],
...
for vname, default, desc in L1OspDir.CONFIG_VARIABLES:
    setattr(
        L1OspDir,
        vname,
        property(fget=partial(L1OspDir._gattr, name=vname, default=default), doc=desc),
    )
