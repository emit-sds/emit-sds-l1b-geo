...
class EmitTimeTable(EmitTimeTableBase):
    ...
    def __init__(
        self, tt_fname, number_sample=1242, reverse_image=False, trim_pad=None
    ):
        ...
        if os.path.splitext(tt_fname)[1] == ".nc":
            ...
            if not (t[0] == list(range(len(t[0])))).all():
                raise RuntimeError(
                    f"For file {tt_fname} first column should be sequential line numnbers"
                )
    ...
    def write_file_txt(cls, tt_fname, tt):
        ...
        tm = np.array(
            [
                int(tt.time(geocal.ImageCoordinate(i, 0))[0].gps * 1e9)
        ...
        with open(tt_fname, "w") as fh:
            for i, t in enumerate(tm):
                print(
                    "%06d %d %s -1 -1" % (i, t, str(geocal.Time.time_gps(t * 1e-9))),
                    file=fh,
                )
    ...
    def write_file_netcdf(cls, tt_fname, tt):
        ...
        tm = np.array(
            [
                tt.time(geocal.ImageCoordinate(i, 0))[0].j2000
                for i in range(tt.max_line + 1)
            ]
        )
        ...
        t.attrs["units"] = "s"
        t.attrs["description"] = (
            "J2000 time for each line in the spectral radiance image"
        )
        fout.close()
