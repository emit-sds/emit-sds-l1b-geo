...
class AvirisNgGeoGenerate:
    ...
    def __init__(self, l1_osp_dir, orb_fname, tt_fname, rad_fname):
        ...
        self.rad = geocal.GdalRasterImage(rad_fname, l1_osp_dir.match_rad_band)
        ...
        self.ipi = geocal.Ipi(
            self.orb, self.cam, 0, self.tt.min_time, self.tt.max_time, self.tt
        )
    ...
    def create_scene(self):
        ...
        for i in range(
            0, max_line // nline_per_scene * nline_per_scene, nline_per_scene
        ):
        ...
        for i, r in enumerate(rset):
            ...
            ipisub = geocal.Ipi(
                self.orb, self.cam, 0, ttsub.min_time, ttsub.max_time, ttsub
            )
            igcsub = geocal.IpiImageGroundConnection(
    ...
    def run(self):
        ...
        standard_metadata = AvirisNgStandardMetadata(
        ...
        if self.igccol_initial.number_image == 0:
            ...
            self.cam.parameter = self.igccol_corrected.image_ground_connection(
                0
            ).ipi.camera.parameter
        # TODO Put in copy orbit parameter when we have an orbit model that
        # gets updated.

        if self.l1_osp_dir.do_final_projection and self.igccol_initial.number_image > 0:
            proj = L1bProj(
                self.igccol_corrected, self.l1_osp_dir, self.geo_qa, img_type="final"
            )
            proj.proj(pool=pool)
