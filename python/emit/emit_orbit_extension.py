...
def _write_corrected_orbit(self, orbit_fname, orb_corrected):
    ...
    t.attrs["units"] = "s"
    t = g2.create_variable(
        "time_j2000", ("t",), data=np.array([t.j2000 for t in eph_tm])
    )
    t.attrs["units"] = "s"
    t = g.create_variable("eci_position", ("t", "position"), data=pos_uncorr)
    t.attrs["description"] = "ECI position"
    t.attrs["units"] = "m"
    t = g2.create_variable("eci_position", ("t", "position"), data=pos)
    t.attrs["description"] = "ECI position"
    t.attrs["units"] = "m"
    t = g.create_variable("eci_velocity", ("t", "velocity"), data=vel_uncorr)
    t.attrs["description"] = "ECI velocity"
    t.attrs["units"] = "m/s"
    t = g2.create_variable("eci_velocity", ("t", "velocity"), data=vel)
    t.attrs["description"] = "ECI velocity"
    t.attrs["units"] = "m/s"
    ...
    t.attrs["units"] = "s"
    t = g2.create_variable(
        "time_j2000", ("t",), data=np.array([t.j2000 for t in att_tm])
    )
    t.attrs["units"] = "s"
...
def _write_file(cls, orbit_fname, orb, min_time, max_time):
    ...
    tspace = 1.0
    tm = np.arange(min_time.j2000 - tspace, max_time.j2000 + tspace, tspace)
