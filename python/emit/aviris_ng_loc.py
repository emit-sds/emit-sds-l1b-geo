...
class AvirisNgLoc(EnviFile):
    ...
    def map_info_rotated(self, mi):
        ...
        lat = self.latitude
        lon = self.longitude
        pt = [f(geocal.Geodetic(lat[0, i], lon[0, i])) for i in range(self.shape[2])]
        pt.extend(
            f(geocal.Geodetic(lat[-1, i], lon[-1, i])) for i in range(self.shape[2])
        )
        pt.extend(
            f(geocal.Geodetic(lat[i, 0], lon[i, 0])) for i in range(self.shape[1])
        )
        pt.extend(
            f(geocal.Geodetic(lat[i, -1], lon[i, -1])) for i in range(self.shape[1])
        )
