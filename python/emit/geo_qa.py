...
class GeoQa:
    ...
    def __init__(self, fname, log_fname, l1a_att_fname, tt_and_rdn_fname, l1_osp_dir):
        self.fname = fname
        ...
        fout.create_group("Orbit Correction")
    ...
    def dataframe(cls, qa_fname):
        ...
        t = pd.DataFrame(
            [t.decode("utf-8") for t in fh["Tiepoint/Scene Times"][:]],
            columns=[
                "Scene Times",
            ],
        )
        t2 = pd.DataFrame(
            fh["Tiepoint/Tiepoint Count"][:],
            columns=[
                "Initial Tiepoint",
                "Blunders",
                "Number Tiepoint",
        ...
        t7 = pd.DataFrame(
            [
                qa_val[v]
                for v in fh["Accuracy Estimate/Geolocation accuracy QA flag"][:]
            ],
            columns=[
                "QA Flag",
            ],
    ...
    def igccol_initial(cls, qa_fname, include_img=True):
        ...
        for i in range(len(rdn_fname)):
            tt_and_rdn_fname.append(
                (tt_fname[i].decode("utf8"), rdn_fname[i].decode("utf8"))
            )
        return EmitIgcCollection.create(
            l1a_att_fname,
            tt_and_rdn_fname,
    ...
    def open_fh(self, access="a"):
        ...
        if version.parse(h5netcdf.__version__) >= version.parse("0.13.0"):
            fout = h5netcdf.File(
                self.fname, access, decode_vlen_strings=False, phony_dims="sort"
            )
        else:
    ...
    def add_tp_log(self, scene_name, tplogfname):
        ...
        tplog_group.create_variable(
            scene_name, ("i",), data=log, dtype=h5py.special_dtype(vlen=bytes)
        )
        fh.close()
...
"""
    ...
    def add_final_accuracy(self, igccol_corrected, tpcol):
        ...
        orb_group.create_variable(
            "Parameter", ("i",), data=igccol_corrected.orbit.parameter
        )
        fh.close()
        # Ok if no tiepoints for scene i, this just return nan
        self.tp_stat[:, 5] = np.array(
            [
        ...
        for i in range(igccol_corrected.number_image):
            ...
            self.tp_stat[i, 6] = t1
            self.tp_stat[i, 7] = t2
            qa_val = self.l1_osp_dir.geocal_accuracy_qa(t1, t2, self.tp_stat[i, 2])
    ...
    def close(self):
        ...
        f = self.open_fh()
        log_group = f["Logs"]
        log_group.create_variable(
            "Overall Log", ("i",), data=log, dtype=h5py.special_dtype(vlen=bytes)
        )
        tp_group = f["Tiepoint"]
        if self.scene_name is not None:
            tp_group.create_variable(
                "Scene Times",
                ("i",),
        ...
        if self.tp_stat is not None:
            dset = tp_group.create_variable(
                "Tiepoint Count", ("i", "c"), data=self.tp_stat[:, 0:4].astype(np.int32)
            )
            dset.attrs[
                "Description"
...
Fourth column is the number to image matching tries we did."""
        ...
        if self.tp_stat is not None:
            dset = ac_group.create_variable(
                "Accuracy Before Correction", ("i",), data=self.tp_stat[:, 4]
            )
            dset.attrs["Units"] = "m"
            dset = ac_group.create_variable(
                "Final Accuracy", ("i",), data=self.tp_stat[:, 5]
            )
            dset.attrs["Units"] = "m"
            dset = ac_group.create_variable(
                "Delta time correction before scene", ("i",), data=self.tp_stat[:, 6]
            )
            dset.attrs["Units"] = "s"
            dset = ac_group.create_variable(
                "Delta time correction after scene", ("i",), data=self.tp_stat[:, 7]
            )
            dset.attrs["Units"] = "s"
            dset = ac_group.create_variable(
                "Geolocation accuracy QA flag", ("i",), data=self.tp_stat[:, 8]
            )
            dset.attrs[
                "Description"
